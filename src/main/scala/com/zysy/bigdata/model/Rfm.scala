...
object Rfm {
  def main(args: Array[String]): Unit = {
    ...
    spark.read.option("header", "true").csv("data/customer.csv").
      select("customer_id", "total_cost_fee", "total_repair_times", "after_days").
      createTempView("v1")
    ...
    val dataModel = modelUsers.transform(userProfile)
    val silhouette = evaluator.evaluate(dataModel)
    println(s"silhouette=$silhouette")
